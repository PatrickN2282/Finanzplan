...
def calculate_months(start_date, end_date):
    if not end_date:
        return 1  # Einmalig, aber für Finanzierungen sollte end_date vorhanden sein
        ...
        if not df.empty:
            for _, row in df.iterrows():
                ...
                if s <= target and (not e or target <= e):
                    ...
                    # Berechne monatlichen Betrag basierend auf betrag_typ
                    if row['betrag_typ'] == "Gesamtbetrag":
                        num_months = calculate_months(row['start_datum'], row['end_datum'])
                        monthly_amount = row['betrag'] / num_months
                    else:  # "Monatliche Rate"
                        monthly_amount = row['betrag']
                    val_actual = monthly_amount if is_due else 0
                    val_anteilig = monthly_amount * mapping.get(row['intervall'], 1)
                    if row['typ'] == "Einnahme":
                        ein_ist += val_actual
                    ...
                    table_data.append({"Monat": m_label, " ": get_emoji(row['art'], row['typ']), "Konto": row['konto_name'], "Zweck": row['zweck'], "Kategorie": row['kategorie'], "Betrag (fällig)": val_actual, "Anteilig p.M.": val_anteilig, "Turnus": row['intervall'], "Typ_Internal": row['typ'], "Ist_Fällig": is_due})
