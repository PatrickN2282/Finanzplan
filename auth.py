        ...
        with st.form("reg_form"):
            ...
            if st.form_submit_button("Account erstellen", width='stretch'):
                try:
                    ...
                    c.execute("INSERT INTO users (username, password, vorname, nachname) VALUES (?,?,?,?)", (new_u, hash_pw(new_p), new_vorname, new_nachname))
                    u_id_new = c.lastrowid
                    # Standard-Kategorien für neuen User anlegen
                    for kat in ["Gehalt", "Miete", "Lebensmittel", "Auto", "Versicherung"]:
                        c.execute("INSERT INTO kategorien (user_id, name) VALUES (?,?)", (u_id_new, kat))
                    # Standard-Konto anlegen
                    c.execute("INSERT INTO konten (user_id, name, iban, typ) VALUES (?, ?, ?, ?)", (u_id_new, "Hauptkonto", "", "Bankkonto"))
